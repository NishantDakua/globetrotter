...
const MyTrips = () => {
  // State for Currently Exploring trip
  ...
  return (
    <div className="min-h-screen bg-[#0b0c10] text-gray-100 flex flex-col md:flex-row antialiased selection:bg-teal-500/30 selection:text-teal-200">
      ...
      <Sidebar 
        onOpenNewTrip={() => setActiveModal({ type: 'new-trip', data: null })} 
      />
          ...
          {currentlyExploring && (
            <section className="space-y-4">
              <CurrentTripCard
                ...
                onViewLiveItinerary={() => setActiveModal({ type: 'live-itinerary', data: currentlyExploring })}
          ...
          <section className="space-y-4 pt-2">
            ...
            {upcomingTrips.length === 0 ? (
              <div className="bg-[#141622] border border-white/10 rounded-2xl p-8 text-center space-y-3">
                ...
                <button
                  onClick={() => setActiveModal({ type: 'new-trip', data: null })}
                  className="px-4 py-2 bg-[#009688] text-white text-xs font-medium rounded-xl hover:bg-[#008477] transition"
                >
